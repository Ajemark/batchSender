...
async function readJettonWalletMedata(client: TonClient, address: string) {
  const walletData = await client.callGetMethod(
...
async function readNftItemMetadata(client: TonClient, address: string) {
  ...
  const nftData = await client.callGetMethod(nftItemAddress, "get_nft_data");
  ...
  const nftCollectionContent = await client.callGetMethod(
    collection_address,
    "get_nft_content",
    [arg1, arg2]
  );
...
async function readJettonMetadata(
  client: TonClient,
  address: string
): Promise<{
  persistenceType: persistenceType;
